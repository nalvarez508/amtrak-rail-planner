...
class AmtrakSearch:
  ...
  def preSearchSetup(self, origin, destination, departDate):
    self.origin = origin
    self.destination = destination
    self.departDate = departDate
  ...
  def checkEveryPage(self, area, pages):
    for page in range(1,pages+1):
      self.updateStatusMessage(f"Searching - checking page {page}")
  ...
  def oneWaySearch(self):
    ...
    try: # Loading the page
      self.updateStatusMessage("Searching - loading page")
      ...
      try: # Clicking the new search button
        self.updateStatusMessage("Searching - opening input fields")
        ...
        try: # Entering to/from stations
          self.updateStatusMessage("Searching - entering stations")
          ...
          try: # Entering departure date
            self.updateStatusMessage("Searching - entering travel dates")
            self.enterDepartDate(searchArea)

            # Wait until "Find Trains" button is enabled, then click it
            self.updateStatusMessage("Searching - retrieving results")
            ...
            try:
              self.updateStatusMessage("Searching - loading results")
              time.sleep(1)
              self.updateStatusMessage("Searching - checking results")
            ...
            # Search has been completed, but we didn't find any trains on that day
              ...
              # Search has been completed, and we found a train(s)
                ...
                try:
                  self.updateStatusMessage("Searching - parsing results page")
